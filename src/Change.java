...
public class Change {
    ...
    public static void main(String[] args) {
        ...
        int n=sc.nextInt();//지폐 갯수 값
        int change=sc.nextInt();//거스름돈
        int arr[]=new int [n];//지폐종류값 할당 배열 ex)1000원 50000원 등등등
        int name[]=new int[n];//"~~원은 ~~장이 필요합니다."-> 출력할때 ~~원 출력 담당 배열
        for(int i=0;i<n;i++){
            arr[i]=sc.nextInt();//거슬러줄 지폐종류 입력
            name[i]=arr[i];//출력할때 필요
        }

        int cnt=0; //지폐,동전 개수 카운트 변수
        for(int i=n-1;i>=0;i--)//큰 화폐단위부터 봐줘야 하기 때문에 i를 n-1부터 줄어들게함
        {
